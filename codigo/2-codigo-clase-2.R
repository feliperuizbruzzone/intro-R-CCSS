...
#Supogamos un estudio de opinión sobre medidas gubernamentales COVID-19.
#Crearemos una base de datos de 9 casos y 3 variables
  #Género: 1 = Hombre/ 0 = Mujer. 
  #Ingreso: cantidades de pesos chilenos.
  #Acuerdo: escala de acuerdo con medidas gobierno COVID-19
...
pandemia <- data.frame(genero, ingreso, acuerdo) #resulta como objeto de "datos" en entorno

View(pandemia) # Para visualizar base de datos
                   # O botón en objeto base de datos que se observa en entorno de trabajo

dim(pandemia) # Nos dice las dimensiones de la base de datos

names(pandemia) # Indica los nombres de las variables (columnas)

class(pandemia) # Señala de qué tipo es el objeto

#Cálculo de estadísticos simples de variables específicas ("$ como indicador de posición)

#Tabla de frecuencias simple
table(pandemia$genero)

#Media
mean(pandemia$ingreso)
...
saveRDS(pandemia, file = "datos/2-pandemia.rds")
