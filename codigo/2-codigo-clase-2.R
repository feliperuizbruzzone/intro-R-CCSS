...
# ESTACIÓN LASTARRIA - NOVIEMBRE 2021
...
#Supongamos un estudio de opinión sobre el proyecto de cuarto retiro fondos de pensiones.
#Crearemos una base de datos de 9 casos y 3 variables
  #Género: 1 = Hombre/ 0 = Mujer. 
  #Ingreso: cantidades de pesos chilenos.
  #Acuerdo: escala de acuerdo cuarto retiro fondos de pensiones
...
retiro <- data.frame(genero, ingreso, acuerdo) #resulta como objeto de "datos" en entorno

View(retiro) # Para visualizar base de datos
                   # O botón en objeto base de datos que se observa en entorno de trabajo

dim(retiro) # Nos dice las dimensiones de la base de datos

names(retiro) # Indica los nombres de las variables (columnas)

class(retiro) # Señala de qué tipo es el objeto
...
saveRDS(retiro, file = "datos/2-retiro.rds")
