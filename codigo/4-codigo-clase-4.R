...
install.packages(c("ggplot2", "knitr", "stargazer"))

# En caso de buscar ejecutar RMarkdown en RStudio Offline
# install.packages("tynitex")
# tinytex::install_tinytex(force = TRUE) #instalar aplicación de latex para uso de RMarkdown.

# ---- 2. ESTABLECER CARPETA DE TRABAJO, CARGAR DATOS Y PAQUETES ----

# CARGAR BASES DE DATOS Y PAQUETES

load("datos/medias_atmos.RData")#C  ARGAR BASE "MEDIAS_ATMOS" - INFORMACIÓN METEROLÓGICA
load("datos/mtcars.RData") #CARGAR BASE "MTCARS" INFORMACIÓN TÉCNICA DE AUTOS
load("datos/economics.RData") # CARGAR BASE "ECONOMICS" - DATOS DE EMPLEO EEUU
load("datos/UDP.RData")
